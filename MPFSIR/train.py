...
def train(config):
    ...
    for epoch_number in range(EPOCHS):
        ...
        vims, _ = eval_vim(config, test_loader, model, save=False)
        test_vim_avg = np.mean(vims)
        if best_vim_avg > test_vim_avg:
            best_vim_avg = test_vim_avg
            torch.save(model.state_dict(), './models/{0}/{1}_best.pt'.format(config.dataset, args.out_model_name))
...
if __name__ == '__main__':
    ...
    import os
    if not os.path.exists("./models/"+config.dataset):
        os.makedirs("./models/"+config.dataset, exist_ok=True)

    if not args.out_model_name:
        args.out_model_name = ""
        
    model = train(config)

    if args.out_model_name and args.out_model_name != "":
        model_path = './models/{0}/{1}.pt'.format(config.dataset, args.out_model_name)
