...
def eval_vim(config, ds_loader, model, EVAL_FRAMES=[2, 4, 8, 10, 14], save=True):
    ...
    if save:
        #np.save("../data/predictions/3dpw_mpfsir", y_pred)
        np.save("../data/predictions/{0}_mpfsir".format(config.dataset), y_pred)
...
def test_handball_shot(config, args):
    from dataset import create_datasets
    from model import create_model
    
    _, _, test_loader, _ = create_datasets(config=config)
    ...
    print("[Test] VIM: {0} - {1}".format(test_vim, round(np.mean(test_vim), 2)))
...
def test_cmu_mupots(config, args):
    from dataset.dataset_cmu_mupots import create_datasets
    from model import create_model
...
if __name__ == '__main__':
    ...
    #args.out_model_name = "./models/{0}/{1}.pt".format(config.dataset, args.out_model_name)
    ...
    if args.dataset == "3dpw":
        ...
        test_handball_shot(config, args)
    else:
        config.input_len = 15
