...
from dataset.amass import AMASSDataset

def create_3dpw_test_out_json_if_needed():
    if os.path.exists("../data/somof/3dpw_test_out.json"):
        return
    
    ds, _ = load_3dpw(split="test", out_type="dict")
    
    with open("../data/somof/3dpw_test_in.json") as f:
        X = np.array(json.load(f))
    with open("../data/somof/3dpw_test_frames_in.json") as f:
    ...
    with open("../data/somof/3dpw_test_out.json", "w") as outfile:
...
class SoMof3DPW(Dataset):
    ...
    def __init__(self, config, dir_path="../data/somof/", name="test"):
        self.config = config
