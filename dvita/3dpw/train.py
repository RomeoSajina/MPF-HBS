...
def main(args):
    ...
    if args.ckp != None and len(args.ckp) > 0:
    ...
    for epoch in range(args.n_epochs):
        ...
        for idx, (obs_p, obs_s, obs_f, target_p, target_s, target_f, start_end_idx) in enumerate(train):
            ...
            ade_train += float(utils.ADE_c(preds_p.detach(), target_p.detach()))
            fde_train += float(utils.FDE_c(preds_p.detach(), target_p.detach()))
            vim_train += utils.myVIM(preds_p.detach(), target_p.detach())
        ...
        print('Saving ...', epoch)
        torch.save(net_g.state_dict(), './models/{0}/checkpoint_g_{1}.pkl'.format(args.dataset, epoch))
        torch.save(net_l.state_dict(), './models/{0}/checkpoint_l_{1}.pkl'.format(args.dataset, epoch))
        
        if best_vim > vim_val:
            best_vim = vim_val
            print('Saving ...', best_vim, epoch)
            torch.save(net_g.state_dict(), './models/{0}/checkpoint_g_{1}.pkl'.format(args.dataset, "best_epoch"))
            torch.save(net_l.state_dict(), './models/{0}/checkpoint_l_{1}.pkl'.format(args.dataset, "best_epoch"))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckp', default='', type=str, required=False)
