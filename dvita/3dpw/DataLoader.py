...
class myJAAD(torch.utils.data.Dataset):
    def __init__(self, args):
        ...
        if args.dtype == "train":
            ...
            self.data_in = np.concatenate((self.data_in, x_amass[:, :, :16]), axis=0)
            self.data_out = np.concatenate((self.data_out, x_amass[:, :, 16:]), axis=0)

        
        print("\n\n", args.dtype, np.array(self.data_in).shape, np.array(self.data_out).shape)
    ...
    def __getitem__(self, idx):
        ...
        return obs_p, obs_s, None, target_p, target_s, None

    
class myJAADExPI(torch.utils.data.Dataset):
    def __init__(self, args):

        print('Loading', args.dtype, 'data from handball_shot...')
        
        data = np.load("../../data/handball_shot/{0}.npy".format(args.dtype))
        
        self.data_in =  data[:, :, :16].reshape(-1, 2, 16, 39)
        self.data_out = data[:, :, 16:].reshape(-1, 2, 14, 39)
...
def data_loader(args):
    ...
    dataset = myJAAD(args) if args.dataset == "3dpw" else myJAADExPI(args)
