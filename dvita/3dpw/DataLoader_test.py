...
class myJAADhandball_shot(torch.utils.data.Dataset):
    def __init__(self):
        
        print('Loading test data from handball_shot...')

        data = np.load("../../data/handball_shot/test.npy")

        self.data_in =  data[:, :, :16].reshape(-1, 2, 16, 39)
        self.data_out = data[:, :, 16:].reshape(-1, 2, 14, 39)
        
        print("\n\n", np.array(self.data_in).shape, np.array(self.data_out).shape)

    def __len__(self):
        return len(self.data_in) 
    
    def __getitem__(self, idx):
        obs_keypoints = torch.tensor(self.data_in[idx])
        obs_speed_keypoints = (obs_keypoints[:,1:,:] - obs_keypoints[:,:-1,:]) 
        return obs_keypoints, obs_speed_keypoints, None, None
...
def data_loader(args):
    dataset = myJAAD() if args.dataset == "3dpw" else myJAADhandball_shot()
