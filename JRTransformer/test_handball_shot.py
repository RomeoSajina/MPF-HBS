...
import numpy as np
import torch
from dataset.dataset_handball_shot import DS_test, batch_denormalization
...
class Tester:
    def __init__(self, args):
        ...
        print("# Param:", sum(p.numel() for p in self.model.parameters() if p.requires_grad))
    ...
    def test(self):
        ...
        with torch.no_grad():
            for i, data in enumerate(self.test_loader):
                ...
                y_pred.append(motion_pred[:, :, :13, :])
                
                count += batch_size
            
            y_pred = np.concatenate(y_pred, axis=0).reshape(-1, 2, 14, 13, 3)
                
            np.save("../data/predictions/handball_shot_jrtransformer", np.array(y_pred))
