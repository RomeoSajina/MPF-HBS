...
class SoMoFDataset(MultiPersonPoseDataset):
    ...
    def load_data(self):
        ...
        data_out = data_out.reshape(-1, 2, 14, 39)
        
        data = np.concatenate((data_in, data_out), axis=2)
        data = torch.from_numpy(data)
        data = data.reshape((*data.shape[:-1], 13, 3)) # (N, 2, 30, 13, 3)
...
class AMASSDataset(MultiPersonPoseDataset):
    ...
    def load_data(self):
        self.data = load_data_amass(split=self.split, sub=self.sub).numpy()[:, 0]
...
# <handball_shot>
import os
def load_my_dataset(split="train", SL=30, freq=2):
    data = np.load("../data/handball_shot/{0}.npy".format(split))
    print("Loading {0} split of dataset:".format(split), data.shape)
    return data
        
class MyDataset(MultiPersonPoseDataset):

    def __init__(self, **args):
        super(MyDataset, self).__init__("handball_shot", frequency=1, **args)

    def load_data(self):
        
        data = load_my_dataset(split=self.split) # (N, 2, 30, 13, 3)
...
def create_dataset(dataset_name, logger, **args):
    ...
    elif dataset_name == "handball_shot":
