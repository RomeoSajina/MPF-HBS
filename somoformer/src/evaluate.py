...
def myVIM(pred, GT, calc_per_frame=True, return_last=True):
    if calc_per_frame:
...
def evaluate_vim(model, dataloader, config, logger, return_all=False, bar_prefix="", vam=False, save_npy=False):
    ...
    for i, batch in enumerate(dataloader):
        ...
        for person in range(num_people):
            ...
            for k in range(len(out_joints)):
                ...
                scene_people.append([person_pred_joints])
        ...
        if save_npy:
            scene_people = np.array(scene_people)
            N = scene_people.shape[0]//2
            y_pred = np.concatenate((scene_people[:N], scene_people[N:]), axis=1)
            
            y_pred = y_pred.reshape(-1, 2, 14, 13, 3) + pelvis.transpose(1, 2).unsqueeze(-2).cpu().numpy()

            pred_results.append(y_pred)
            
            #np.save("../data/predictions/3dpw_somoformer", np.array(y_pred))
    ...
    if save_npy and dataloader.dataset.name != "3dpw":
        #np.save("../data/predictions/3dpw_somoformer", np.array(y_pred))
        ds_name = "3dpw" if dataloader.dataset.name == "somof" else "handball_shot"
        np.save("../data/predictions/{0}_somoformer".format(ds_name), np.concatenate(pred_results, axis=0))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--handball_shot", action="store_true", help="Run handball_shot validation instead of 3dpw")
    ...
    elif not args.handball_shot:
        dataset = create_dataset("3dpw", logger, split=args.split, track_size=(in_F+out_F), track_cutoff=in_F, segmented=True)
        dataloader = DataLoader(dataset, batch_size=config['TRAIN']['batch_size'], num_workers=config['TRAIN']['num_workers'], shuffle=False, collate_fn=collate_batch)
        
    elif args.handball_shot:
        logger.info("Using handball_shot data")
        in_F, out_F = config['TRAIN']['input_track_size'], config['TRAIN']['output_track_size']
        dataset = create_dataset("handball_shot", logger, split=args.split, track_size=(in_F+out_F), track_cutoff=in_F, segmented=True)
        dataloader = DataLoader(dataset, batch_size=config['TRAIN']['batch_size'], num_workers=config['TRAIN']['num_workers'], shuffle=False, collate_fn=collate_batch)


    if args.metric == 'vim':
        avgs = evaluate_vim(model, dataloader, config, logger, return_all=True, save_npy=True)
