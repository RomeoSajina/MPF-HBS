...
def train(config, logger, experiment_name="", ckpt=None):
    ...
    if config["DATA"]["joints"] == "somof" and config['DATA']['train_datasets'][0] == "handball_shot":
        # 3dpw validation set for val metrics
        dataset_3dpw_train_segmented = create_dataset("handball_shot", logger, split="train", track_size=(in_F+out_F), track_cutoff=in_F, segmented=True)
        dataloader_3dpw_train_segmented = dataloader_for(dataset_3dpw_train_segmented, config, shuffle=True, pin_memory=True)
        dataset_3dpw_val = create_dataset("handball_shot", logger, split="valid", track_size=(in_F+out_F), track_cutoff=in_F, segmented=True)
        ...
        dataset_somof_train = create_dataset("handball_shot", logger, split="train", track_size=(in_F+out_F), track_cutoff=in_F, segmented=True)
        dataset_somof_val = create_dataset("handball_shot", logger, split="valid", track_size=(in_F+out_F), track_cutoff=in_F, segmented=True)
        ...
        dataset_somof_test = create_dataset("handball_shot", logger, split="test", track_size=(in_F+out_F), track_cutoff=in_F, segmented=True)
        dataloader_somof_test = dataloader_for(dataset_somof_test, config, shuffle=False)
                
    elif config["DATA"]["joints"] == "somof":
        # 3dpw validation set for val metrics
        ...
        dataset_somof_test = create_dataset("somof", logger, split="test", track_size=(in_F+out_F), track_cutoff=in_F, segmented=True)
        dataloader_somof_test = dataloader_for(dataset_somof_test, config, shuffle=False)
    ...
    for epoch in range(config["TRAIN"]["epochs"]):
        ...
        if not config['DATA']['segmented'] and config['DATA']['train_datasets'][0] != "handball_shot":
        ...
        test_somof_vim_3dpw = evaluate_vim(model, dataloader_somof_test, config, logger, return_all=True, bar_prefix="3dpw test")
        test_somof_vim_3dpw_avg = test_somof_vim_3dpw[[1, 3, 7, 9, 13]].mean()
        
        if best_avg_vim > test_somof_vim_3dpw_avg:
            best_avg_vim = test_somof_vim_3dpw_avg
            save_checkpoint(model, optimizer, epoch, config, 'best_checkpoint.pth.tar', logger)
...
if __name__ == "__main__":
    ...
    train(cfg, logger, experiment_name=args.exp_name, ckpt=args.ckpt)
