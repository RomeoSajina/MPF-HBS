...
def path_to_repo(*args): # REPO/arg1/arg2
    return os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), *args)
...
def path_to_config(*args): #REPO/src/configs/arg1/arg2
...
def init_output_dirs(experiment, exp_name="default"):
    log_dir = path_to_experiment(experiment)
    ckpt_dir = os.path.join(log_dir, "checkpoints", "checkpoint_"+exp_name)
...
def load_config(path, exp_name="default"):
    ...
    if "OUTPUT" not in config:
        config["OUTPUT"] = {}
    config["OUTPUT"]["log_dir"], config["OUTPUT"]["ckpt_dir"], config["OUTPUT"]["runs_dir"] = init_output_dirs(config["EXPERIMENT"], exp_name=exp_name)
