...
class TBIFormer(nn.Module):
    ...
    def __init__(
            ...
            device='cuda', kernel_size=10, opt=None, pose_dim=39, output_len=14):
        ...
        self.mlp = nn.Sequential(nn.Conv1d(in_channels=pose_dim, out_channels=d_model, kernel_size=kernel_size1,
                                             bias=False),
                                   nn.ReLU(inplace=False),
        ...
        self.proj_inverse=nn.Linear(d_model, pose_dim)
        self.l1=nn.Linear(d_model, d_model*8)
        self.l2=nn.Linear(d_model*8, d_model*output_len)
    ...
    def forward(self, src, n_person, trj_dist):
        ...
        index = [[0, 2, 4], [1, 3, 5], [7, 9, 11], [8, 10, 12], [0, 1, 6]]  # 5 body parts
        ...
        dec_output = dec_output.view(bn, self.output_len, -1)
