...
def run_model(is_train, model, dataloader, optimizer, opt):
    ...
    for batch_id, batch_data in tqdm(enumerate(dataloader)): 
        ...
        root_pos = input_[:, 1:, 0:3]  # root index is 11, get 3D positions of pelvis joint
        ...
        aligned_pred = pred_poses - pred_poses[:, :, :, 0:1, :]   # remove global translation
        aligned_gt = gt_poses - gt_poses[:, :, :, 0:1, :]
...
def processor(opt):
    ...
    dataset = Data(dataset=opt.dataset, split='train', scene=None, device=device,  input_time=opt.input_time, output_time=opt.output_time)
    
    test_dataset = Data(dataset=opt.dataset, split='test', scene=None, device=device,  input_time=opt.input_time, output_time=opt.output_time)
    ...
    for epoch_i in range(1, opt.epochs+1):
        ...
        if Evaluate:
            with torch.no_grad():
                ...
                print_errors(opt.dataset+"_test", ret['gjpe_err'], ret['ajpe_err'], ret['root_err'])
