...
def run_model(is_train, model, dataloader, opt):
    ...
    for batch_i, batch_data in tqdm(enumerate(dataloader)):
        ...
        y_test.append(gt_poses.cpu().numpy())
        y_pred.append(pred_poses.cpu().numpy())
    ...
    np.save("../data/predictions/{0}_socialtgcn".format(opt.dataset), y_pred)
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        ret = run_model(is_train=1, model=model, dataloader=test_dataloader, opt=opt)
